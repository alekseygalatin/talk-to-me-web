...
const ForgotPasswordForm: React.FC<ForgotPasswordFormProps> = ({
  ...
  const [step, setStep] = useState<ResetStep>(ResetStep.RequestReset);
  ...
  const handleRequestReset = async (event: React.FormEvent) => {
    ...
    const requestData = JSON.parse(
      localStorage.getItem("forgotPasswordAttempts") || "{}"
    );
    const now = Date.now();
    ...
    if (requestData[email] && requestData[email].count >= limit) {
      const timeSinceFirstAttempt =
        now - requestData[email].firstAttemptTimestamp;
    ...
    try {
      ...
      setError("");
      setStep(ResetStep.SubmitNewPassword); // Move to the next step
    } catch (err: any) {
  ...
  const handleResetPassword = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError("");
      setStep(ResetStep.RequestReset); // Reset to the first step
